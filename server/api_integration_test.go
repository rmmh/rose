...
func TestPreparedWriteRetryFollowsRename(t *testing.T) {
	s := newServer(t)
	ctx := context.Background()
	original, err := s.Open(ctx, &pb.OpenRequest{Path: "/temporary-prepared", OperationKey: "rename-prepared-retry"})
	require.NoError(t, err)
	_, err = s.Rename(ctx, &pb.RenameRequest{OldPath: "/temporary-prepared", NewPath: "/final-prepared"})
	require.NoError(t, err)

	retry, err := s.Open(ctx, &pb.OpenRequest{Path: "/final-prepared", OperationKey: "rename-prepared-retry"})
	require.NoError(t, err)
	_, err = s.Write(ctx, &pb.WriteRequest{Handle: retry.GetHandle(), Buffer: []byte("resumed")})
	require.NoError(t, err)
	_, err = s.Close(ctx, &pb.CloseRequest{Handle: retry.GetHandle(), IdempotencyKey: "rename-prepared-retry"})
	require.NoError(t, err)
	assert.Equal(t, []byte("resumed"), readAll(t, s, "/final-prepared"))
	_, err = s.Close(ctx, &pb.CloseRequest{Handle: original.GetHandle(), IdempotencyKey: "rename-prepared-retry"})
	require.NoError(t, err)
}
...
func TestMkdirCannotDescendFromPendingFile(t *testing.T) {
	client := newClient(t)
	ctx := context.Background()
	open, err := client.Open(ctx, &pb.OpenRequest{Path: "/parent", OperationKey: "pending-parent"})
	if err != nil {
		t.Fatal(err)
	}
	if _, err := client.Write(ctx, &pb.WriteRequest{Handle: open.GetHandle(), Buffer: []byte("file")}); err != nil {
	...
	if _, err := client.Mkdir(ctx, &pb.MkdirRequest{Path: "/parent/child"}); err == nil {
		t.Fatal("Mkdir succeeded below an unpublished file")
	}
	if _, err := client.Close(ctx, &pb.CloseRequest{
		Handle: open.GetHandle(), IdempotencyKey: "pending-parent",
	}); err != nil {
		t.Fatalf("pending parent did not remain publishable: %v", err)
	}
	attr, err := client.Getattr(ctx, &pb.GetattrRequest{Path: "/parent"})
	if err != nil {
		t.Fatal(err)
	}
	if attr.GetIsDir() || attr.GetSize() != int64(len("file")) {
		t.Fatalf("published parent = dir:%v size:%d, want file size %d", attr.GetIsDir(), attr.GetSize(), len("file"))
	}
}
...
func TestReplaceDiskUsesPreAttachedEmptyDestination(t *testing.T) {
	...
	if _, err := srv.AddDisk(ctx, &pb.AddDiskRequest{
		DiskId: 2, NodeId: 2, TotalBytes: 2 << 30,
	}); err == nil {
		t.Fatal("AddDisk retry changed the disk's declared capacity")
	}
...
func TestRemoveDiskRetryReturnsCompletedJob(t *testing.T) {
	...
	first, err := srv.RemoveDisk(ctx, req)
	if err != nil {
		t.Fatal(err)
	}
	retry, err := srv.RemoveDisk(ctx, req)
	if err != nil {
		t.Fatalf("identical RemoveDisk retry failed: %v", err)
	}
	if retry.GetJobId() != first.GetJobId() {
		t.Fatalf("RemoveDisk retry job = %d, want %d", retry.GetJobId(), first.GetJobId())
	}
}
