...
func (c *virtualScaleCluster) rebalance(maxMoves int, minSkew int64) int {
	...
	for moves < maxMoves {
		...
		for _, p := range plogsOf(src) {
			...
			epoch, err := c.db.CapturePlogRelocation(ctx, p.PlogID, p.VlogID, src)
			if err != nil {
				c.t.Fatal(err)
			}
			destination, err := c.db.CaptureDiskPlacement(ctx, dst)
			if err != nil {
				c.t.Fatal(err)
			}
			if _, err := c.db.MovePlogToDisk(ctx, p.PlogID, p.VlogID, src, dst, epoch, destination.Epoch); err != nil {
				c.t.Fatal(err)
			}
			usage[src] -= p.Length
			usage[dst] += p.Length
