...
| `Publish` | `publishPreparedVersion`, `DB.CommitWriteOpVersion` | Runtime validates canonical complete extents, scoped content, all shards, and persisted prefixes. The model omits those representations and SQLite failure/ambiguous-reply boundaries. |
