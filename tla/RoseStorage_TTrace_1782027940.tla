---- MODULE RoseStorage_TTrace_1782027940 ----
...
_inv ==
    ~(
        ...
        /\
        node_state = ([d1 |-> "working", d2 |-> "failed", d3 |-> "working", d4 |-> "working"])
        /\
...
\* to `JsonSerialize`. For example, a sub-sequence of _TETrace.
    \* ASSUME
    \*     LET J == INSTANCE Json
    \*         IN J!JsonSerialize("RoseStorage_TTrace_1782027940.json", _TETrace)
...
\*
\*trace == IODeserialize("RoseStorage_TTrace_1782027940.bin", TRUE)
\*
\*=============================================================================
\*
...
trace == 
    ...
    ([job_progress |-> [j1 |-> FALSE],disk_state |-> [d1 |-> "failed", d2 |-> "active", d3 |-> "active", d4 |-> "absent"],pending |-> [d1 |-> {}, d2 |-> {}, d3 |-> {}, d4 |-> {}],job_disk |-> [j1 |-> "none"],job_state |-> [j1 |-> "idle"],plog_ready |-> {"d1", "d2"},job_kind |-> [j1 |-> "none"],last_rpc |-> "FailNode",stored |-> [d1 |-> {}, d2 |-> {<<"o1", 0>>}, d3 |-> {}, d4 |-> {}],node_state |-> [d1 |-> "working", d2 |-> "failed", d3 |-> "working", d4 |-> "working"],object_state |-> [o1 |-> "committed"],file_state |-> [o1 |-> "buffered"],object_mode |-> [o1 |-> "DUPLICATE"]])
    >>
----
...
---- CONFIG RoseStorage_TTrace_1782027940 ----
CONSTANTS
    Nodes = { "d1" , "d2" , "d3" , "d4" }
    Disks = { "d1" , "d2" , "d3" , "d4" }
...
ALIAS
    _expression
=============================================================================
\* Generated on Sun Jun 21 01:45:59 MDT 2026